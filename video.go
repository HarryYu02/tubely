...
func getAspectRatio(width, height int) string {
	aspectRatio := math.Round(float64(width) / float64(height) * 100.0) / 100.0
	if aspectRatio == roundToTwoDecimals(16.0 / 9.0) {
		return "16:9"
	} else if aspectRatio == roundToTwoDecimals(9.0 / 16.0) {
		return "9:16"
	} else {
		return "other"
	}
